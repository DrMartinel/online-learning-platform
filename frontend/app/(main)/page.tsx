...
async function getCourses(token: string | undefined): Promise<Course[]> {
  try {
    ...
    const res = await fetch(`${backendUrl}/courses`, { headers, cache: "no-store" });
    if (!res.ok) return [];
...
async function getCurrentUser(token: string | undefined): Promise<Record<string, string> | null> {
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
          ...
          <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {user.role === "admin" ? (
              <Link
                href="/admin/exams"
                className="group relative overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/80 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-amber-500/40 dark:hover:border-amber-500/30 transition-all duration-300 flex flex-col gap-3 scale-100 hover:scale-[1.01]"
              >
              ...
              <div className="relative overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/80 rounded-2xl p-5 shadow-sm flex flex-col gap-3">
          ...
          {user.role === "admin" && (
            <div className="mt-6 flex items-center gap-2 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/30">
              <ShieldAlert size={16} className="text-amber-600 dark:text-amber-400 shrink-0" />
              <p className="text-xs text-amber-700 dark:text-amber-300">
                Bạn đang đăng nhập với quyền <strong>Quản trị viên</strong>. Truy cập{" "}
                <Link href="/admin" className="underline font-semibold">Admin Dashboard</Link>{" "}
                để quản lý hệ thống.
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-zinc-400 dark:text-zinc-500">
              {myCourses.length === 0
                ...
                : `Bạn đang tham gia ${myCourses.length} khóa học`}
            </p>
          </div>
          {myCourses.length > 0 && (
            <Link
        ...
        {myCourses.length === 0 ? (
          <div className="bg-white dark:bg-zinc-900/50 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl p-12 text-center max-w-2xl mx-auto">
            ...
            <h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-2">
              Bắt đầu hành trình học tập mới
            </h3>
            <p className="text-sm text-zinc-400 dark:text-zinc-500 mb-6 max-w-md mx-auto">
              Bạn chưa đăng ký bất kỳ khóa học nào trên hệ thống. Hãy khám phá ngay kho tàng khóa
              học chất lượng cao của chúng tôi.
            </p>
            <Link
              href="/courses"
              className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-6 text-sm font-bold text-white shadow-md shadow-indigo-600/10 hover:bg-indigo-500 transition"
            >
